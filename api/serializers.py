...
class FlightSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        validated_data['aircraft_type'] = Aircraft.objects.get(user=self.initial_data.get(
            'user'), aircraft_type=self.initial_data.get('aircraft_type'))
        validated_data['registration'] = TailNumber.objects.get(user=self.initial_data.get(
            'user'), registration=self.initial_data.get('registration'))
        ...
        for approach in approaches:
            ...
            if 'id' in approach:
                Approach.objects.filter(
                    id=approach.get('id')).update(**approach)
            else:
                Approach.objects.create(flight_object=flight, approach_type=approach.get(
                    'approach_type'), number=approach.get('number'))
...
class TailNumberSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TailNumber
        fields = ['id', 'user', 'registration',
                  'aircraft', 'is_121', 'is_135', 'is_91']
