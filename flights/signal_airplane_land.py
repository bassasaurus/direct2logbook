...
def amel_update(sender, instance, **kwargs):
    ...
    flight = Flight()
    cat_class_query = Q(aircraft_type__aircraft_class__aircraft_class__icontains='multi engine land') & Q(
    ...
    if not simulated_instrument.get('simulated_instrument__sum'):
        ...
        amel.simulated_instrument = simulated_instrument.get(
    ...
    night = Flight.objects.filter(user=user).filter(
        cat_class_query, night__gte=0).aggregate(Sum('night'))
    ...
    landings_night = Flight.objects.filter(user=user).filter(
        cat_class_query).aggregate(Sum('landings_night'))
    ...
    last_60 = Flight.objects.filter(user=user).filter(user=user).filter(
        cat_class_query).filter(date__lte=today, date__gte=last_60).aggregate(Sum('duration'))
    ...
    last_90 = Flight.objects.filter(user=user).filter(user=user).filter(
        cat_class_query).filter(date__lte=today, date__gte=last_90).aggregate(Sum('duration'))
    ...
    last_180 = Flight.objects.filter(user=user).filter(user=user).filter(
        cat_class_query).filter(date__lte=today, date__gte=last_180).aggregate(Sum('duration'))
    ...
    last_yr = Flight.objects.filter(user=user).filter(user=user).filter(
        cat_class_query).filter(date__lte=today, date__gte=last_yr).aggregate(Sum('duration'))
    ...
    last_2yr = Flight.objects.filter(user=user).filter(user=user).filter(
        cat_class_query).filter(date__lte=today, date__gte=last_2yr).aggregate(Sum('duration'))
    ...
    ytd = Flight.objects.filter(user=user).filter(user=user).filter(
        cat_class_query).filter(date__lte=today, date__gte=jan_1).aggregate(Sum('duration'))
...
def asel_update(sender, instance, **kwargs):
    ...
    cat_class_query = Q(aircraft_type__aircraft_class__aircraft_class__icontains='single engine land') & Q(
        aircraft_type__aircraft_category__aircraft_category__icontains='airplane')
    ...
    if not simulated_instrument.get('simulated_instrument__sum'):
        ...
        asel.simulated_instrument = simulated_instrument.get(
            'simulated_instrument__sum')
    ...
    last_30 = today - datetime.timedelta(days=30)
    last_30 = Flight.objects.filter(user=user).filter(user=user).filter(
        cat_class_query).filter(date__lte=today, date__gte=last_30).aggregate(Sum('duration'))
    if not last_30.get('duration__sum'):
        asel.last_30 = 0
    else:
