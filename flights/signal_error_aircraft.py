...
def aircraft_error(sender, instance, **kwargs):
    ...
    if not instance.turbine and not instance.piston:
        ...
        Aircraft.objects.filter(user=user).filter(pk=instance.pk).update(power_error=power_error)
    else:
        Aircraft.objects.filter(user=user).filter(pk=instance.pk).update(power_error='')

    if not instance.simple and not instance.compleks:
        config_error = "Please select 'Simple' or 'Complex'"
        Aircraft.objects.filter(user=user).filter(pk=instance.pk).update(config_error=config_error)
    else:
